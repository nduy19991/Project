import React from "react";
import Styles from "./TaskBar..module.css"
import { HiOutlineUser } from 'react-icons/hi';
import { AiOutlineHeart, AiOutlineShopping } from 'react-icons/ai';

const TaskBar: React.FC = () => {
    return (
        <div>
            <div className={Styles.top_bar}></div>
            <div className={Styles.mid_bar}>
                <div className={Styles.mid_main}>
                    <a className={Styles.mid_linklogo} href="https://www.asos.com/women/">
                        <img className={Styles.mid_logo} src="data:image/svg+xml;base64," />
                    </a>
                    <div className={Styles.mid_btn}>
                        <button className={Styles.mid_btnwomen}>
                            <a className={Styles.mid_link} href="https://www.asos.com/women/">WOMEN</a>
                        </button>
                        <button className={Styles.mid_btnwomen}>
                            <a className={Styles.mid_link} href="https://www.asos.com/men/">MEN</a>
                        </button>
                    </div>
                    <div className={Styles.mid_search}>
                        <input
                            className={Styles.mid_input}
                            type="text"
                            placeholder="Search for itmes and brands"
                        />
                    </div>
                    <div className={Styles.widgets}>
                        <button className={Styles.btn_widgets}><HiOutlineUser className={Styles.icon_widgets}/></button>
                        <button className={Styles.btn_widgets}><AiOutlineHeart className={Styles.icon_widgets}/></button>
                        <button className={Styles.btn_widgets}><AiOutlineShopping className={Styles.icon_widgets}/></button>
                    </div>
                </div>
            </div>
            <div className={Styles.bot_bar}>Xám</div>
        </div>
    )
}

export default TaskBar